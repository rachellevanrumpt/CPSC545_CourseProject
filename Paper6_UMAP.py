...
if __name__ == "__main__":
    # Initialize data
    data = pd.read_csv('..\\Project\\paper6_data\\Cells.csv') 
    data.dropna(subset=["umap1"], inplace=True)
    data = data.to_numpy()[:,:]
    plotx = np.asarray(data[2:,4], dtype=float) # UMAP X-coordinate
    ploty = np.asarray(data[2:,5], dtype=float) # UMAP Y-coordinate
    original_labels = np.asarray(data[2:,2]) # Original cluster labels
    num_clusters = 13
    eps = 2.5
