...
if __name__ == "__main__":
    # Initialize data
    data = pd.read_csv('..\\Project\\paper9_data\\SCC\\Cells.csv') # SCC or BCC
    data.dropna(subset=["umap1"], inplace=True)
    data = data.to_numpy()[:,:]
    plotx = np.asarray(data[2:,6], dtype=float) # UMAP X-coordinate
    ploty = np.asarray(data[2:,7], dtype=float) # UMAP Y-coordinate
    original_labels = np.asarray(data[2:,4]) # Original cluster labels #4 for SCC 3 for BCC
    num_clusters = 10
    eps = 0.7 #5 for BCC and 0.7 for SCC
