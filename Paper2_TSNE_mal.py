...
def DBSC(cells, umap1, umap2, set_esp):
    start_time = timeit.default_timer() # Time algorithm
    dbscan = DBSCAN(eps=set_esp, min_samples=5)
...
if __name__ == "__main__":
    # Initialize data
    data = pd.read_csv('..\\Project\\paper2_data\\P2_tsne_mal.csv').to_numpy()[:,:]
    plotx = np.asarray(data[2:,1], dtype=float) # TSNE X-coordinate
    ploty = np.asarray(data[2:,2], dtype=float) # TSNE Y-coordinate
    original_labels = np.asarray(data[2:,3]) # Original cluster labels
    set_esp = 8
    num_clusters = 13
    ...
    plt.figure(figsize=(6, 4))
    colors = plt.cm.Spectral(np.linspace(0, 1, num_clusters))
    for x, z_val in zip(range(len(plotx)), numbered_labels):
        color = colors[z_val] if z_val < len(colors) else 'violet'
        plt.scatter(plotx[x], ploty[x], s=50, color=color, edgecolor='k')
    ...
    # DBSCAN
    dbscan_labels = DBSC(cells,plotx,ploty,set_esp)
